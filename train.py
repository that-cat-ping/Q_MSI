...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=25, clinical=True):
    since = time.time()
    ...
    if clinical:
        clinical_feature=pd.read_csv('/home/ubuntu/stomachpredictor/stomachpredictor/label/STAD2.csv')
        peoples = [i for i in clinical_feature.id]
        features=[clinical_feature[i] for i in clinical_feature.columns[1:]]
