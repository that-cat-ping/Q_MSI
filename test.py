...
def test(model, model_name, k=0, K=10, types=0, cnv=False):
    model.eval()

    if cnv:
        cnv_feature=pd.read_csv('/home/ubuntu/stomachpredictor/stomachpredictor/data/livetmb/test_{k}.csv')  # 改动1
    ...
    testset = CustomDset(os.getcwd()+f'/data/stomoachTMB/test_{k}.csv', data_transforms['test'])  # 改动2
    ...
    np.save(os.getcwd()+f'/results/stomachtmb/y_true_{k}.npy', np.array(y_true))  # 改动地方
    np.save(os.getcwd()+f'/results/stomachtmb/score_{k}.npy', np.array(score_list))
    np.save(os.getcwd()+f'/results/stomachtmb/y_pred_{k}.npy', np.array(y_pred))
